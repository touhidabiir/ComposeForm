...
| `language` | string | `"en"` | One of `"en"` or `"bn"`. Selects both the numeral script used when `numbered` is `true` (`"bn"` renders question numbers as Bangla numerals, `১, ২, ৩...`, instead of Arabic digits) and the font every field in this schema renders with — `"en"` uses Inter, `"bn"` uses Noto Sans Bengali (fetched at runtime via the Google Fonts provider; requires Google Play Services on the device). Does not translate any other text; field/option labels are still authored directly in whichever language the JSON already uses. |
