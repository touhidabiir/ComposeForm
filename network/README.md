...
- `api/` — the Retrofit service interfaces, one per base URL (`AppApiService`, `PaymentApiService`, `AnalyticsApiService`, `PartnerApiService`, and any added later — see "Multiple base URLs" below). Each is a flat interface of `suspend fun` endpoint methods for that base URL, public (needed by `:data`'s `*RepositoryImpl` classes), bound as a Hilt-injectable singleton in `NetworkModule`. Adding a new endpoint to an existing base URL is a new method on its interface, not a new file. `AppApiService` is the one backing the app's actual real screens today; the other three are placeholder shapes for now. `:app` never references any of them directly — by convention, not compiler enforcement — go through `:data`'s repositories instead.
- `model/` — the request/response data classes, Gson-reflected, a growing collection rather than a fixed set. A model whose JSON fields already happen to be camelCase needs no serialization annotations (exact field-name matching); one mirroring a real backend's snake_case JSON shape 1:1 uses `@SerializedName` per field instead. Grouped one file per feature/endpoint, not one file per class — a new endpoint's models are a new file here, not entries added to existing ones.
