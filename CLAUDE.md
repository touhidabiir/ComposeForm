...
- `HeaderInterceptor` (`network/.../HeaderInterceptor.kt`) — `internal`, takes a `Map<String, String>` and adds each as a header via `intercept`; the reusable way to give one base URL a static header (e.g. a partner's API key) by passing an instance into `RetrofitFactory.create()`'s `interceptors` list.
