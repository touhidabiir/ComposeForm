...
- `network/.../auth/` — `TokenProvider` (the contract; storage is deferred to the consuming module, same pattern as `@BaseUrl`), `AuthInterceptor` (`internal`, attaches `Authorization: Bearer <token>` when a token is present), and `NoAuth` — a method annotation for a Retrofit service function whose requests must never carry the token (e.g. a third-party/partner endpoint). `AuthInterceptor` checks for it via Retrofit's own `Invocation` request tag (`request.tag(Invocation::class.java)?.method()?.isAnnotationPresent(NoAuth::class.java)`), so the opt-out lives on the method declaration rather than at each call site; it fails safe (attaches the token unless explicitly annotated, never the reverse). `:app` binds `TokenProvider` to `di/EncryptedTokenProvider.kt` (backed by `androidx.security.crypto.EncryptedSharedPreferences`, so the token survives app relaunch) via `di/AppTokenModule.kt`.
...
- **Multiple base URLs**: beyond the main `@BaseUrl`/`AppApiService`/`AppRepository` trio, `:network` has three more base-URL-scoped API groups following the identical pattern — `@PaymentBaseUrl`/`PaymentApiService`/`PaymentRepository`, `@AnalyticsBaseUrl`/`AnalyticsApiService`/`AnalyticsRepository`, and `@PartnerBaseUrl`/`PartnerApiService`/`PartnerRepository` (placeholder names/endpoints for now). Each is its own Hilt qualifier (`network/.../PaymentBaseUrl.kt` etc., public, bound by `:app`'s `AppNetworkModule`) plus an internal Retrofit-instance qualifier private to `NetworkModule` (`RetrofitQualifiers.kt` — `@PaymentRetrofit`/`@AnalyticsRetrofit`/`@PartnerRetrofit`) used only to disambiguate the multiple `Retrofit` singletons from each other. All four `Retrofit` instances share the *same* `OkHttpClient` singleton (one connection pool/dispatcher for the whole app, not one per base URL) — `PartnerApiService`'s methods are `@NoAuth` rather than needing a separate unauthenticated client. Extending this further (a 5th base URL) means repeating this same five-file shape: a public `@XBaseUrl` qualifier, an internal `@XRetrofit` qualifier, an `internal XApiService`, a public `XRepository`, and the two `@Provides` functions in `NetworkModule` — never reuse another group's `@BaseUrl`/qualifier for an unrelated backend.
- `NetworkModule` (`network/.../NetworkModule.kt`) — `internal`, wires the `OkHttpClient` (30s connect/read/write timeouts, `AuthInterceptor` + debug-gated `HttpLoggingInterceptor` — `Level.BODY` only when `:network`'s own generated `BuildConfig.DEBUG` is true, `Level.NONE` otherwise so release builds never log request/response bodies), the `Retrofit`/`*ApiService` singletons for every base URL above; not visible outside the module. `:network` enables `buildFeatures.buildConfig` itself for this — deliberately not a Hilt qualifier like `@BaseUrl`, since Gradle's variant-aware dependency resolution already gives every module the `BuildConfig.DEBUG` matching whichever build type of `:app` is being built, with no value needing to cross the module boundary.
