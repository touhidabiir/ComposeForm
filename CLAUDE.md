...
- `@BaseUrl` (`network/.../BaseUrl.kt`) — a Hilt qualifier a consuming module's own Hilt module binds to a `String` (e.g. `@Provides @BaseUrl fun provideBaseUrl(): String = "..."`) so `:network` never hardcodes an environment's base URL. Once product flavors exist, `:app` reads this from a per-flavor `BuildConfig.BASE_URL` — `:network` itself never needs to know about flavors. `:app` currently supplies a dummy URL via `di/AppNetworkModule.kt`.
